	...
	if tai[index].raw and tai[index].gameid then
		for k,v in pairs(tai[index].gameid) do
			...
			elseif #v.prx < 1 then -- No have any plug?, remove!!!!
				...
				tai.delete_sect(index, v) -- Remove al sections of id...
				tai.parse(index)
...
function tai.del(index, id, path)
	...
	if idx then
		...
		if #tai[index].gameid[id].prx == 1 and (id != "KERNEL" or id != "main" or id != "ALL") then -- remove section if not have nothing more prx!
